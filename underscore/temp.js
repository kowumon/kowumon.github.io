...
(function () {
    ...
    // Baseline setup
    // --------------
    // 윈도우 객체는 모든 브라우저에 지지한다. 브라우저의 윈도우를 나타내고 모든 global 자바스크립트 객체,함수,변수는 자동적으로 윈도우 객체의 멤버가 된다.
    // --------------
    ...
    // All **ECMAScript 5** native function implementations that we hope to use(사용하길 희망하는 실행함수 )
    // are declared here.
    var nativeIsArray = Array.isArray, // 배열인지 확인하고 true /false를 반환
    ...
    var optimizeCb = function (func, context, argCount) {
        if (context === void 0) return func; // context가 undefined 면 함수를 반환
        switch (argCount) { // argCount는 어디서 셀까
            case 1:
                return function (value) {
                    return func.call(context, value); // obj.call(this, arg); 객체를 흡수한다.
                };
            // The 2-parameter case has been omitted only because no current consumers
            // made use of it. ? 개의 파라미터 케이스는 생략된다 오직 왜냐하면 현재 소비자가 그것을 사용할 수 없게 만들었기 때문에
            case null:
            case 3:
    ...
    //콜렉션에서 각 요소에 가해질 수 있는 콜백을 일으키는 내부함수
    // 인데 여기서 내부함수는 아이덴티티, 멋대로인 콜백 macher property,프로퍼티 접근자를 이든지 바라는 결과를 리턴하는 내부함수 를 말한다.
    ...
    // `_.iteratee` if they want additional predicate/iteratee shorthand styles. ( 단정하다/ 반복하다 ) 추가적인 것을 원한다면 '_iteratee'를 커스터마즈할지 모른다. 콜백 제너레이터에 대한 External wrapper이다.
    // This abstraction hides the internal-only argCount argument.
    // 이것은 추상은 내부적인 유일한 argCount 인자를 숨긴다.
  ...
  /*
   배열 또는 객체로서 반복해야하는 콜렉션인지 아닌지를 결정하는 콜렉션에 대한 메서드를 돕는 helper
   */
    ...
    }; // predicate에 맞는 결과가 없다면 'undefiend'
